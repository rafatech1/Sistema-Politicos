...
/**
 * Seção escura de fechamento — espelha o hero e dá arco visual à home
 * (escuro → claro → escuro). A lógica do formulário (honeypot, LGPD,
 * estados) está intocada; só a apresentação mudou.
 */
export function ContactSection({ socialData }: { socialData: SocialLinksData }) {
  ...
  return (
    <section id="contato" className="relative overflow-hidden bg-slate-950 px-6 py-20 text-white sm:py-28">
      <div
        aria-hidden
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute -left-32 top-0 h-96 w-96 rounded-full bg-primary opacity-20 blur-3xl"
      />

      <div className="relative mx-auto grid max-w-6xl gap-12 sm:grid-cols-2 sm:items-start">
        <div>
          <p className="mb-4 inline-block -rotate-1 border-2 border-slate-900 bg-accent px-2.5 py-1 text-xs font-bold uppercase tracking-wide text-slate-900">
            Fale comigo
          </p>
          <h2 className="font-display text-3xl font-black uppercase leading-[0.95] tracking-tight sm:text-5xl">
            Sua voz faz parte disso
          </h2>
          ...
          <SocialIcons
            ...
            iconClassName="flex h-11 w-11 items-center justify-center rounded-full border-2 border-white/30 text-white transition-all hover:-translate-y-0.5 hover:border-white hover:bg-white hover:text-slate-900"
          />
        </div>
        ...
        <div className="relative">
          ...
          <form
            ...
            className="relative space-y-4 border-2 border-slate-900 bg-white p-6 sm:p-8"
          >
            <input
              type="text"
              name="website"
